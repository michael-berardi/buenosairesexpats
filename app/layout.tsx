...
const luceroReferralTrackingScript = `(function(){var dashboardEndpoint='https://www.libertydesign.studio/api/network/observability/client-event';var base={event_group:'referral',referral_type:'spoke_to_hub',source_site_id:'buenosairesexpats',source_network_id:'lucero',source_site_role:'spoke',source_segment:'audience-satellite',hub_site_id:'lucero-legal',destination_site_id:'lucero-legal',destination_network_id:'lucero',destination_site_role:'hub',destination_segment:'hub'};function cleanText(value){return (value||'').replace(/\\s+/g,' ').trim().slice(0,120);}function sendDashboardEvent(data){var payload=JSON.stringify({schema:'lds.network.client-event.v1',siteId:base.source_site_id,eventName:'lucero_referral',timestamp:Date.now(),path:window.location.pathname,data:data});try{if(navigator.sendBeacon&&navigator.sendBeacon(dashboardEndpoint,new Blob([payload],{type:'text/plain;charset=UTF-8'}))){return;}}catch(_error){}try{fetch(dashboardEndpoint,{method:'POST',mode:'no-cors',keepalive:true,headers:{'Content-Type':'text/plain;charset=UTF-8'},body:payload});}catch(_error){}}document.addEventListener('click',function(e){var a=e.target&&e.target.closest?e.target.closest('a[href*="lucerolegal"]'):null;if(!a){return;}var data=Object.assign({},base,{link_url:a.href,link_text:cleanText(a.textContent),page_path:window.location.pathname,language:document.documentElement.lang||'en'});sendDashboardEvent(data);if(window.va){window.va('event',{name:'lucero_referral',data:data});}if(window.gtag){window.gtag('event','lucero_referral',data);}});})();`;
